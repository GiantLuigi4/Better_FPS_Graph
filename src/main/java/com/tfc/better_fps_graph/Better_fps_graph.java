	...
	private void onDrawFPSGraph(RenderGameOverlayEvent event) {
		if (event instanceof RenderGameOverlayEvent.Pre) {
			if (event.getType() == RenderGameOverlayEvent.ElementType.FPS_GRAPH) {
				...
				Minecraft.getInstance().fontRenderer.drawString(new MatrixStack(), Minecraft.getInstance().gameSettings.framerateLimit + " FPS", max1, y1 - ((1000f / Minecraft.getInstance().gameSettings.framerateLimit) * 2) - 3, 255);
				Minecraft.getInstance().fontRenderer.drawString(new MatrixStack(), "Minimum " + min + " ms", max1, y1 - (min * 2) - 3, 65280);
				Minecraft.getInstance().fontRenderer.drawString(new MatrixStack(), "Maximum " + max + " ms", max1, y1 - (max_render) - 3, 16711680);
				Profiler.reset();
